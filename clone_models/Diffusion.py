...
from clone_utils.utils import default;
...
class GaussianDiffusion(nn.Module):
    def __init__(self, 
        ...
        self.res = res;
        self.aux_percep_loss = aux_percep_loss;
        ...
        self.register_buffer('posterior_variance', to_torch(posterior_variance)) #Beta_hat
    ...
    def forward(self, img_hr, img_lr, img_lr_up, t=None, *args, **kwargs):
        ...
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long() \
            if t is None else torch.LongTensor([t]).repeat(b).to(device) #[B,1]
        if self.use_rrdb:
            if self.fix_rrdb:
                self.rrdb.eval()
                with torch.no_grad():
    ...
    def p_losses(self, x_start, t, cond, img_lr_up, noise=None):
        ...
        # print(type(x_tp1_gt))
        # print("art",x_tp1_gt.shape,"cond",len(cond),cond[0].shape,"lr up",img_lr_up.shape);
