...
from clone_trainer.Trainer import Trainer;
...
class SRDiffTrainer(Trainer):
    ...
    def build_model(self):
        ...
        denoise_fn = Unet(
            hidden_size, out_dim=3, cond_dim=32, dim_mults=dim_mults);
        
        rrdb_ckpt = "";
        use_rrdb = True;
        if use_rrdb:
            rrdb = RRDBNet(3, 3, 32, 8,32 // 2)
            if rrdb_ckpt != '' and os.path.exists(rrdb_ckpt):
                load_ckpt(rrdb, rrdb_ckpt)
        else:
            rrdb = None;
    ...
    def build_scheduler(self, optimizer):
        return torch.optim.lr_scheduler.StepLR(optimizer, 100_000, gamma=0.5)
